...
export class FetchResource implements BaseResource {
  ...
  public setHeaders(headers: object) {
    this.defaultOptions.headers = Object.assign(
      {},
  ...
  public getAllEntities(): Promise<any> {
    return new Promise((_, reject) => {
      reject(
        new Error("BaseHttpResource #getAllEntities(): need to provide method")
      );
    });
  }
  ...
  private async handleError(response: Response) {
    ...
    this.defaultOptions?.handleError &&
  ...
  private resolveRequestData(
    ...
    const queryParams =
      method === "get" ? body : options && options.queryParams;
    ...
    requestUrl =
      query != null && query != "" ? `${requestUrl}?${query}` : requestUrl;
  ...
  private resolveRequestUrl(url: string, o?: FetchOptions): string {
    if (this.baseUrl == null) {
      throw new Error(
        "BaseHttpResource#resolveRequestUrl: baseUrl is not defined"
      );
    }
  ...
  public getQueryString(params: any = {}, o?: FetchOptions): string {
    const options =
      o != null
        ...
        : (this.defaultOptions as FetchOptions);
